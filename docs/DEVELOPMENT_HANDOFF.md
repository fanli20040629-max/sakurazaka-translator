...
> 接手前必读：[本轮优化验收与交接](本轮优化验收与交接.md) 第 0 节及第 7～10 节。先核对实际源码和环境；每项“通过”附实际验证结果；发现矛盾先记录、核实，不凭旧文档或模型判断宣称完成。该入口还提供代码定位表、故障处理顺序和交付模板。

> 最新：0.5.0-translation-trial / 6，文本翻译试用代码已接入，新增 INTERNET。按 [当前交接](本轮优化验收与交接.md) 构建与测试；下方离线、无网络权限和 0.4 的要求均为历史阶段。

> 2026-09-17：先读 [本轮优化验收与交接](本轮优化验收与交接.md)。当前源码 `0.4.0-probe-candidates` / 5 已接入节点/OCR 统一候选；Windows 构建及 C01～C13 真机验证仍待执行。以下指令、路径和任务进度保留为历史参考，不应重新执行已完成的旧重构。
