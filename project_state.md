...
- 当前里程碑：2026-09-13 已完成 M0，M1 第一批已实施并接入生产服务；M2～M5 尚未完成。已新增生产结构模型、正文整理器和任务协调器，接入节点结构采集、OCR 完成监听、节点异常终止、用户关闭失效和 overlay 窗口登记。桌面开发已到真机调试边界；仍不开发翻译、API Key、缓存或后台自动识别。
- 恢复点：此前 6 个 Markdown 已提交为 e90497b；生产代码仍为 b7dcdca 对应版本。test-result.png 保留，不提交、不删除。
- 本次验证：扩展后的 verify-probe-logic.ps1 / ProbeLogicSelfTest PASS；`:app:compileDebugJavaWithJavac --offline`、`:app:lintDebug`、`:app:assembleDebug` PASS（仅过时 API/Gradle 版本和 aapt2 实验性提示）；APK badging、权限、v2 签名、OCR native/model 检查 PASS。ADB 已检查，当前无设备。
...
- 下一步：使用本次 APK 进行手机安装、无障碍授权和合成页验证；根据真实回调和视觉结果继续 M2～M4。目标 App 四类页面在合成页通过后执行。保守回退允许待复测 APK 交付，不代表 P0.5 通过。
