...
- 当前里程碑：M0 已完成，M1 第一批已实施并接入生产服务；2026-09-14 已收到首批新版目标页真机视觉证据。M2～M5 尚未完成。节点长文和 OCR 都能产出结果，下一阶段转入正文分类、来源对照和候选选择界面；仍不开发翻译、API Key、缓存或后台自动识别。
- 恢复点：方案提交 `ee80f90`，当前代码与文档检查点 `aed21f2`。`test-result.png` 保留，不提交、不删除。
- 本次验证：扩展后的 verify-probe-logic.ps1 / ProbeLogicSelfTest PASS；`:app:compileDebugJavaWithJavac --offline`、`:app:lintDebug`、`:app:assembleDebug` PASS（仅过时 API/Gradle 版本和 aapt2 实验性提示）；APK badging、权限、v2 签名、OCR native/model 检查 PASS。2026-09-14 用户提供的两张目标页截图显示指定窗口截图成功（`1080×2340`）、节点长文可读、日文 OCR 可运行且一次样本耗时 `836ms`。截图未展示 APK 哈希，因此不能仅据图片独立确认安装包身份。
...
- 下一步：以本次真机结果为输入继续 M2～M4：将正文、作者/时间、控件和媒体候选分开；隐藏默认坐标诊断；把 OCR 改为独立候选并支持用户选择。完成新 APK 后再复测长文、重复、截断、切页和锁屏矩阵。保守回退允许待复测 APK 交付，不代表 P0.5 通过。
