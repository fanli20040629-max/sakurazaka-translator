...
源码 `0.5.0-translation-trial` / 6。在下方候选版基础上接入保守节点分组、符号占位保护、带 ID 的译文校验、DeepSeek 文本适配器、取消/超时、Key 加密、两个可编辑风格档案及原文确认/中文卡片。当前需要 INTERNET，但不发送图片。九组逻辑测试通过；接口测试使用模拟连接，不使用真实 Key。Android 构建、真实接口与真机全部待验证。当前交接见 [本轮优化验收与交接](docs/本轮优化验收与交接.md)；本轮交付分支为 `codex/node-candidates`，接手时核对远程分支和实际提交。下方 0.4 与更早条目按历史读取。
