...
- 当前里程碑：2026-09-13 已完成代码与官方资料核对、M0 的旧自测复跑及方案修订；M1～M5 尚未实施。本轮用户要求制定方案，没有改生产代码、构建新 APK 或进行手机操作。仍不开发翻译、API Key、缓存或后台自动识别。
- 恢复点：此前 6 个 Markdown 已提交为 e90497b；生产代码仍为 b7dcdca 对应版本。test-result.png 保留，不提交、不删除。
- 本次验证：ProbeLogicSelfTest PASS（仅原有逻辑断言）；SDK 36 公开 API 与缓存 Tasks 完成监听已核对；旧 APK SHA-256 与 1B8C9B66EFE45189B05E382DFE54E2A7839D3C250B39E81062034B4E1A676799 一致。未重跑 lint/构建，未检查当前 ADB。
- 优先缺口：节点读取异常可能遗留 busy；OCR 取消缺终止监听；用户关闭与请求失效未统一；内容/窗口事件处理过宽。先修协调器基础，再整理正文。API 33+ 节点 recycle 已无实际作用，验收应关注引用生命周期，不以补调用次数宣称修复。
...
- 下一步：开发前确认构建/签名基线，从 M1 数据契约、生产协调器和结构节点采集开始，再 M2 分类排序去重、M3 OCR 对照、M4 卡片/事件、M5 检查及手机复测。保守回退允许待复测 APK 交付，不代表 P0.5 通过。
